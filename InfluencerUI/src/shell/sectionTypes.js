 ...
 *
 * <p>This file is the single source of truth for what a section IS: which types exist, which
 * fields each one has, and which designed variants it can be shown in. The editor builds its
 * panel from this, and `LandingService.renderSection` renders the same names on the server. When
 * they disagree, a brand fills in a field that never appears on their page — which is worse than
 * a missing feature, because it looks like it worked.
 *
 * <p><b>No field here is a colour, font, size or position.</b> That is the entire mechanism by
 * which the editor "cannot look wrong": layout lives in the stylesheet keyed by type and variant,
 * so the brand picks a designed arrangement instead of authoring one. If a field for a hex value
 * ever appears in this file, the curated editor has become a box canvas again.
 *
 * <p>This module is duplicated into `InfluencerContentUI/src/sectionTypes.js` because the remotes
 * cannot import across project roots and production serves the remote. `shell/remoteCopies.test.mjs`
 * fails when the two drift — copy the change across rather than relaxing the assertion.
 */
