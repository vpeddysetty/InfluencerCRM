 ...
 *
 * <p><b>What makes this different from the builder it replaces.</b> GrapesJS is a free-form box
 * canvas: the user positions things, so page quality becomes a function of their design skill.
 * Here the user picks a section type and fills in named fields, and every arrangement they can
 * reach is one that was designed. There is no way to express "three pixels left".
 *
 * <p><b>Three deliberate omissions</b>, each of which a general-purpose builder would include:
 * no layer tree, no style manager, and <b>no drag-and-drop reordering</b>. Reordering is up/down
 * buttons because they are keyboard reachable, work on touch, and cannot half-succeed — dragging
 * is the interaction that breaks on a phone, which is where a creator asked to tweak their page
 * will be.
 *
 * <p><b>The preview is the real renderer.</b> The canvas shows server-rendered HTML in an iframe
 * rather than a React approximation of it, so what the brand sees is what visitors get. A
 * client-side mock of the page would drift from the Java renderer the first time either changed,
 * and the drift would be invisible until someone published.
 *
 * <p>This component is duplicated into `InfluencerContentUI/src/components/SectionEditor.jsx`
 * (production serves the remote). Change both.
 */
