package com.influencer.finance.security;
...
public class ServiceTokenFilter extends OncePerRequestFilter {
    ...
    public ServiceTokenFilter(@Value("${finance.service-token:}") String expectedToken) {
        this.expectedToken = expectedToken;
        if (expectedToken == null || expectedToken.isBlank()) {
            log.error("finance.service-token is not configured. The finance service will reject all requests. "
                    + "Set finance.service-token here and web-experience.dao-service-token on the BFF to the same value.");
        }
    }
        ...
        if (!isAuthorized(request)) {
            ...
            response.getWriter().write(
                    "{\"status\":401,\"error\":\"Unauthorized\","
                            + "\"message\":\"A valid service token is required to call the finance service\"}");
            return;
        }
