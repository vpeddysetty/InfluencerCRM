package com.influencer.attribution.security;
...
public class ServiceTokenFilter extends OncePerRequestFilter {
    ...
    public ServiceTokenFilter(@Value("${attribution.service-token:}") String expectedToken) {
        this.expectedToken = expectedToken;
        if (expectedToken == null || expectedToken.isBlank()) {
            log.error("attribution.service-token is not configured. The attribution service will reject all requests. "
                    + "Set attribution.service-token here and web-experience.dao-service-token on the BFF to the same value.");
        }
    }
        ...
        if (!isAuthorized(request)) {
            ...
            response.getWriter().write(
                    "{\"status\":401,\"error\":\"Unauthorized\","
                            + "\"message\":\"A valid service token is required to call the attribution service\"}");
            return;
        }
