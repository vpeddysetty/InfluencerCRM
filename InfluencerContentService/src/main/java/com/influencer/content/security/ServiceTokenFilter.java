package com.influencer.content.security;
...
public class ServiceTokenFilter extends OncePerRequestFilter {
    ...
    public ServiceTokenFilter(@Value("${content.service-token:}") String expectedToken) {
        this.expectedToken = expectedToken;
        if (expectedToken == null || expectedToken.isBlank()) {
            log.error("content.service-token is not configured. The content service will reject all requests. "
                    + "Set content.service-token here and web-experience.dao-service-token on the BFF to the same value.");
        }
    }
        ...
        if (!isAuthorized(request)) {
            ...
            response.getWriter().write(
                    "{\"status\":401,\"error\":\"Unauthorized\","
                            + "\"message\":\"A valid service token is required to call the content service\"}");
            return;
        }
