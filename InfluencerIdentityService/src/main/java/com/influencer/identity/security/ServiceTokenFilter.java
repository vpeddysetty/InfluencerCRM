package com.influencer.identity.security;
...
public class ServiceTokenFilter extends OncePerRequestFilter {
    ...
    public ServiceTokenFilter(@Value("${identity.service-token:}") String expectedToken) {
        this.expectedToken = expectedToken;
        if (expectedToken == null || expectedToken.isBlank()) {
            log.error("identity.service-token is not configured. The identity service will reject all requests. "
                    + "Set identity.service-token here and web-experience.dao-service-token on the BFF to the same value.");
        }
    }
        ...
        if (!isAuthorized(request)) {
            ...
            response.getWriter().write(
                    "{\"status\":401,\"error\":\"Unauthorized\","
                            + "\"message\":\"A valid service token is required to call the identity service\"}");
            return;
        }
