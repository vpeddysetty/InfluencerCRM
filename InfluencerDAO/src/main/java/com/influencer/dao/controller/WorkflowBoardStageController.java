...
import com.influencer.dao.repository.WorkflowBoardStageRepository;
...
public class WorkflowBoardStageController {
    ...
    @GetMapping
    public List<WorkflowBoardStage> findAll(@RequestParam(required = false) UUID userId,
    ...
    public List<WorkflowBoardStage> replace(@RequestBody ReplaceRequest request) {
        if (request == null || request.userId == null || request.boardId == null) {
            throw new IllegalArgumentException("userId and boardId are required.");
        }
