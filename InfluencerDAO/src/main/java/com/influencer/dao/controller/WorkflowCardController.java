...
import com.influencer.dao.repository.WorkflowCardRepository;
...
public class WorkflowCardController {
     ...
     *   userId  - scope to a user
     *   boardId - cards on a specific board
     *   board   - "none" to return only unassigned cards (no board)
     */
    @GetMapping
    public List<WorkflowCard> findAll(@RequestParam(required = false) UUID userId,
                                      @RequestParam(required = false) UUID boardId,
                                      @RequestParam(required = false) String board) {
        if (userId != null && boardId != null) {
            return repository.findByUserIdAndBoardIdOrderByPositionAsc(userId, boardId);
        }
        if (userId != null && "none".equalsIgnoreCase(board)) {
            return repository.findByUserIdAndBoardIdIsNullOrderByCreatedAtDesc(userId);
        }
        if (userId != null) {
    ...
    public WorkflowCard update(@PathVariable UUID id, @RequestBody WorkflowCard card) {
        ...
        existing.setUserId(card.getUserId());
