...
public class ServiceTokenFilter extends OncePerRequestFilter {
    ...
    public ServiceTokenFilter(@Value("${dao.service-token:}") String expectedToken) {
        ...
        if (expectedToken == null || expectedToken.isBlank()) {
            log.error("dao.service-token is not configured. The DAO will reject all requests. "
                    + "Set dao.service-token here and web-experience.dao-service-token on the BFF to the same value.");
        }
    }
