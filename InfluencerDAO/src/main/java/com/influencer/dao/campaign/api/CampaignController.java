package com.influencer.dao.campaign.api;
...
public class CampaignController {
    ...
    @GetMapping
    public List<Campaign> findAll(@RequestParam(required = false) UUID brandId,
                                  @RequestParam(required = false) String campaignType) {
        if (brandId != null && campaignType != null && !campaignType.isBlank()) {
            return repository.findByBrandIdAndCampaignType(brandId, campaignType.trim().toLowerCase());
        }
        if (brandId != null) {
    ...
    public Campaign update(@PathVariable UUID id, @RequestBody Campaign campaign) {
        ...
        existing.setBrandId(campaign.getBrandId());
