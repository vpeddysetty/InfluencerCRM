...
| `OP-33` | **The shell E2E suites cannot judge production, and their output invites the opposite conclusion.** Twelve of sixteen shell `curl` directly at `docker exec influencercrm-postgres psql`, so pointed at prod they assert against an EMPTY LOCAL DATABASE while calling the live API — every such check fails, and reads like a product defect. Two examples from 2026-09-02, both harmless: `e2e_asset_library` B11b "the asset survived the cross-tenant delete attempt" failed while B11 (the actual API refusal) returned **404 correctly**, and `e2e_creator_portal` reported a **500** on a claim because `BRAND_A_ID` came back empty from a database on the wrong machine. Others assert `mock` providers where production deliberately runs real ones (`domains.provider=dns`, `social-provider`) — CLAUDE.md §1 says to check the provider property before debugging the integration, and this is that trap in test form. **The fix is a decision, not a patch:** either give them a `--against-prod` mode that skips DB assertions, or state at the top of each that they are local-stack only and stop running them against deployments. Until then their failures against prod are noise, and noise is what the previous row is about. | 0.5 | Found by running them; recorded because the next person will read the same red output and reach for the product |
