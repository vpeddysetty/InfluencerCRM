...
| `OP-43` | **Production served the shell's bundled pages, not the remotes** — 🟡 **ROOT CAUSE FIXED AND DEPLOYED 2026-09-05; one symptom remains.** **The cause:** `vite.config.js:20` read `process.env.VITE_USE_REMOTES`, but `deploy-ui.sh` writes that variable into `.env.production`, which Vite loads into `import.meta.env` for APPLICATION code and **never** into `process.env` for the config. So it was `undefined` on every production build, the federation plugin was never added, and the shell shipped with no remote wiring at all — silently, with no error and no request to any remote origin. `originRegistry.js` had the same fault one layer down: it reads `import.meta.env`, empty in the config, so every origin fell back to `http://localhost:5176` and a build that DID enable federation would still have pointed production at a dev port. **Both fixed:** the config now uses `loadEnv` and passes the result to `federationRemotes(env)`, and `originRegistry.test.mjs` (4 tests) guards it. **Verified live:** the deployed shell now fetches `creators.tejdux.com/remoteEntry.js` and six further chunks, all 200, where it previously made ZERO requests. **What remains:** the remote's entry loads but its `CreatorsPage` chunk is never fetched, so the shell's bundled copy still renders and `PR-67`'s three filters are still absent in production. The deployed remote bundle is byte-identical to a local build and DOES contain them, and the API returns `niche` correctly — so this is a module-resolution step after the entry loads, not stale code or missing data. **Next:** instrument the `contextPage` import in `routeManifest.js`, which catches its own failure and falls back without surfacing why | 1 | CLAUDE.md §1 assumes production serves remotes; until the remaining half lands, it does not, and every judgement resting on it needs rechecking |
