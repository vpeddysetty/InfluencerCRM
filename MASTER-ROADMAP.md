...
| `OP-43` | **Production serves the shell's bundled pages, not the remotes — `VITE_USE_REMOTES=true` notwithstanding** — found 2026-09-05 by driving `app.tejdux.com` in a real browser with a live account. **The symptom:** the creators page renders correctly with its roster, search and platform filter, and **without `PR-67`'s niche / audience / vetting controls or `PR-66`'s panel** — because those exist only in `InfluencerCreatorsUI`, and the page being served is `InfluencerUI/src/pages/CreatorsPage.jsx`. **What was verified:** the browser makes **zero requests to `creators.tejdux.com`** and logs no error and no `[gateway] remote … unavailable` warning, so this is not the documented fallback path — the shell never attempts the remote. `deploy-ui.sh` wrote `VITE_USE_REMOTES=true` before the build (confirmed in the deploy log), and the deployed bundle DOES carry the specifier `` `mf_creators/CreatorsPage` `` — but contains **zero occurrences of `creators.tejdux.com`**, so the origin was never compiled in and there is nothing to fetch. `remoteEntry.js` at the remote is a normal 141-byte re-export stub, byte-identical to a local build, so the remote is not at fault. **Not yet diagnosed:** why the origin is absent from the bundle when the variable was present at build time. **Why this matters beyond the missing controls:** CLAUDE.md §1 states production serves remotes, and every judgement that follows from it — which copy to edit, what `remoteCopies.test.mjs` is protecting, whether a shell-only fix reaches users — is currently inverted. §2.0b's divergence note is the visible half of this | 1 | Six deployed features are unaffected (all API-level and verified live); what is affected is every UI change made in a remote |
