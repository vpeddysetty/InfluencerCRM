...
| ~~`OP-43`~~ | **Production served the shell's bundled pages, not the remotes** — ✅ **FIXED, DEPLOYED AND LIVE-VERIFIED 2026-09-05.** CLAUDE.md §1 says production serves remotes; it had been false since the shell was written. **Three causes, each hiding the next.** **(1)** `vite.config.js:20` read `process.env.VITE_USE_REMOTES`, but `deploy-ui.sh` writes it into `.env.production` — which Vite loads into `import.meta.env` for APPLICATION code and never into `process.env` for the config — so the federation plugin was never added. `originRegistry.js` had the same fault, defaulting every origin to `http://localhost:5176`. Fixed with `loadEnv`. **(2)** `App.jsx` imported all ten pages **directly** from `./pages/` and rendered those, so no route ever reached `contextPage` — the manifest drove only the nav rail. Fixed by exporting the federated components and importing them from the manifest. **(3)** `contextPage` did `import(/* @vite-ignore */ remoteSpecifier)` — a VARIABLE, with a comment telling the bundler not to touch it — so the plugin could not rewrite it and the browser met a bare specifier it has no import map for. Fixed by passing a thunk containing a literal import. **Each cause masked the one below it, and all three failed in silence:** no error, no warning, and a page that rendered perfectly from the wrong copy. **Live-verified:** `app.tejdux.com/creators` now fetches `creators.tejdux.com/assets/CreatorsPage-*.js`, shows all four `PR-67` controls, and filtering `niche=beauty` keeps Bea and drops Fitz. **Guarded by:** `originRegistry.test.mjs` (4 tests) and `creators-prod-check.mjs`, which drives the deployed page in a browser — the only layer that could have caught this, since the API was correct, the module parsed, and the component rendered | 2 | `remoteCopies.test.mjs` was guarding copies of files the deployed app did not load. It now does |
