...
signup.~~ **Superseded 2026-09-04.** All three have moved, two of them deliberately backwards:

- **Billing is code-complete and now gated to the platform owner** (`PR-63`). Not switched off —
  reachable by one account, refusing everyone else with 404. The distinction matters: it can be
  exercised end to end today without a customer meeting a checkout.
- **The public pricing page now says early access and names no price.** That is a decision, not a
  gap: the product is going to brands and agencies to build a case study BEFORE pricing is set, and
  a price guessed ahead of the first real campaign is hard to walk back. Accounts default to
  `agency` limits so nobody meets a ceiling with no upgrade path to explain it.
- **Something does greet a new signup** — `PR-02` shipped the five-step activation checklist,
  empty states and the welcome email (delivery still waits on `OP-06`).

**The headline itself still holds, and more sharply than before: the remaining gap is commercial.**
There is no unblocked engineering work left on this roadmap. Everything open is either waiting on a
third party the founder must engage, or explicitly deferred until a customer names it (§11.5).
...
| `OP-39` | **The dashboard fetches every row a brand has ever had, on every render** — found 2026-09-04 while deciding `PR-64`'s read path. `AnalyticsService` makes **four unfiltered list calls** (`:64-66` — every sale attribution, every creator, every workflow card) and aggregates **in Java memory**, applying the date range *after* the fetch (`withinRange`, `:214-236`). There is no `sum`, no `group by`, and no date predicate in the query — **the DAO endpoint accepts no date parameters at all** (`InfluencerSaleAttributionController:22-24`), so the BFF could not narrow it even if it wanted to. No pagination and no cap on the repository either. **This is `O(all rows ever)` per dashboard load** and degrades as a customer uses the product — the one performance curve that punishes the most engaged account. `PR-64` would multiply it by the client count: an agency with eight brands triggers 32 unbounded fetches per render. **Fix:** date-range parameters on the DAO list endpoints, `sum`/`group by` in the query, and an index on `(brand_id, occurred_at)` — which does not exist, while `idx_das_brand_day` already sits on the rollup table nothing writes to. Same numbers, strongly consistent, orders of magnitude less data on the wire. **Do this INSIDE `PR-64`**, not after: building the portfolio view on the current read path bakes the defect in at eight times the cost | 1 | A live defect in the shipped single-brand dashboard, independent of the agency work. Not urgent at zero subscribers; urgent the moment one has volume |
| `PR-63` | **Stop selling a price that has not been decided** — ✅ **SHIPPED AND LIVE-VERIFIED 2026-09-04 (v1.0.59).** The product goes to brands and agencies to build a case study BEFORE pricing exists, so nothing may offer a plan, name a limit, or ask for a card. **Nothing was deleted:** `PlanPolicy`, the four 402 paths, Stripe billing and the tier copy in `plan.js` all stay as built and tested — two properties change the posture. `WEBE_DEFAULT_PLAN=agency` is the plan an account with none of its own receives; `WEBE_PLATFORM_OWNER_EMAILS` decides who reaches `/api/billing/*`. **The default exists because the halves conflict otherwise:** remove the pricing page while limits still bite and a prospect meets a 25-creator wall with nothing explaining it and no way to upgrade — worse than any price. It does NOT loosen `PlanPolicy.forKey`, which still fails closed to `free` for an unrecognised STORED value: a typo must not grant everything, and a deliberate default is a different thing from a typo. **`PlatformOwnerPolicy` is not a seventh role** — §5's roles govern what someone may do inside a brand; running the platform is a different question, and putting it in `AccountRole` would drag `impliesAllBrands` and `findAccessibleBrands` into it across three trees. Matched on the **verified JWT email**, never a header. **Empty means NOBODY**, so a forgotten env var closes billing rather than handing every account a checkout. Refusal is **404, not 403**: a 403 confirms a billing system is there and says "not you", raising the question the gate exists to avoid. `/api/billing/webhooks/*` is a different controller and stays open — Stripe calls it unauthenticated, and live-verified it answers 503 (no secret configured) rather than 404. **Live-verified:** landing page carries no free-tier line and no schema.org `"price": "0"`; `/pricing/` reads as early access with zero dollar figures in body, head OR comments; all 8 UIs load with no JS errors. **Reverse by setting both properties back** — no code change, no migration | 1 | Two tests changed rather than relaxed, each recording why the rule moved |
...
| `OP-43` | **Production serves the shell's bundled pages, not the remotes** — 🟡 **ROOT CAUSE FIXED AND DEPLOYED; the second cause is DIAGNOSED, NOT FIXED (2026-09-05).** **Cause 1, fixed:** `vite.config.js:20` read `process.env.VITE_USE_REMOTES`, but `deploy-ui.sh` writes it into `.env.production`, which Vite loads into `import.meta.env` for APPLICATION code and never into `process.env` for the config — so it was `undefined` on every production build and the federation plugin was never added. `originRegistry.js` had the same fault one layer down, defaulting every origin to `http://localhost:5176`. Both fixed with `loadEnv`, guarded by `originRegistry.test.mjs` (4 tests), **deployed and verified**: the shell now fetches `creators.tejdux.com/remoteEntry.js` and six chunks (all 200) where it made zero requests before, and the deployed federation module carries the real `*.tejdux.com` origins. **Cause 2, diagnosed:** it changes nothing visible, because **`App.jsx` imports all ten pages DIRECTLY** — `import CreatorsPage from './pages/CreatorsPage'` (`:13`) — and renders them at `:2438`. `routeManifest.js`'s `contextPage` wrappers, which are what would load a remote, drive **only the nav rail**. So no route has ever gone through federation, `import('mf_creators/CreatorsPage')` answers `TypeError: Failed to resolve module specifier` when asked directly in the live app, and the `[gateway] remote unavailable` warning has never fired because the import is never reached. **What this means:** CLAUDE.md §1's "production serves remotes" has been false since the shell was written, not since a recent regression — and `remoteCopies.test.mjs` is guarding copies of files the deployed app does not load. **The fix is a routing change, not a config one:** route elements must come from the manifest's `component`, which is a real refactor of `App.jsx` and deliberately not attempted at the end of a session | 2 | Six deployed backend features are unaffected and verified live; what is affected is every UI change made in a remote |
...
### 12.2b Read path: transactional store, not a CQRS read model — decided 2026-09-04

**The question asked:** should `PR-64` read from a separate read store built with CQRS, or from the
transactional tables?

**Decision: read from the transactional store, and push the aggregation into SQL.** Not because
CQRS is wrong in general, but because three things make it the wrong purchase *here*:

1. **Zero subscribers.** CQRS earns its keep when read and write load have genuinely different
   shapes at scale. A `sum(...) group by` over an indexed table answers in single-digit
   milliseconds and stays fast well past the first hundred customers.
2. **A read model is eventually consistent, and this data is money.** An agency that approves a
   commission, refreshes, and sees a stale figure files it as a bug. `PR-49` already establishes
   that *unknown must not read as zero*; a lagging dashboard reintroduces exactly that ambiguity
   where it costs most.
3. **The projection substrate here is a known trap.** The outbox relay (§4) is in-process and
   at-least-once, and `EVENTS_RELAY_ENABLED` defaults differently per service — CLAUDE.md warns
   that setting it globally starts a relay in every service against one shared outbox table.
   Building a money-critical projection on that, at zero revenue, is a large new failure surface
   bought for no present benefit.

**What the dashboard should do instead** is stop doing the thing that is actually slow — see
`OP-39`. The win is in the query, not the architecture.

**When to revisit:** when a real customer's dashboard exceeds ~500ms. Measure it; do not predict it.
An agency with hundreds of brands and years of history eventually makes the read-store answer
correct, and `daily_attribution_stats` already exists for it (`V8:144`, with `idx_das_brand_day`
already in place at `V11:380`). The cheap next step at that point is a **nightly materialised
rollup**, not an event-sourced projection: idempotent, trivially rebuildable, and with no
consistency lag on today's numbers if the closed days come from the rollup and today comes from a
live query. That is most of CQRS's benefit at a fraction of its cost.
