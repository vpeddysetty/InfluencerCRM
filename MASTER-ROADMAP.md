...
| `PR-44` | **PARTIAL — the editor is on disk; the emails are not.** **Shipped:** the three creator endpoints (`/hand-back`, `/preview`, `/sections/rewrite`) with `handBack`/`previewForCreator`/`rewriteSectionForCreator` in `InfluencerWebExperience/.../content/application/PageCollaborationService.java`, and the editor screen `InfluencerCreatorPortalUI/src/pages/EditPage.jsx`, which mounts `SectionEditor` from `@influencer/ui` (the extraction is done — one copy, consumed by a Vite alias) with creator-scoped callbacks. `pagesForCreator` now returns `brandName` so the send-back button names the brand, tested in `PageCollaborationSaveTest.listedPagesCarryTheBrandName` and confirmed to fail without it. BFF 550 tests, shell 250, packages/ui 18, plus `tests/e2e/creator-portal.spec.js` driving the whole loop against a running stack. **Verified live, which found two defects the unit tests could not:** the creator rewrite 400d on every call because `rewriteSection` builds a Brief and `BriefEnricher` sources `goal` only from a campaign brief, which most campaigns lack — the brand-side section editor has the same bug, since `SectionEditor`'s `onRewrite` contract carries no brief; and every field in the shared editor was an unlabelled control (bare `<label>`, no `htmlFor`/`id`), so a screen reader announced "edit text, blank" for a page headline on both sides. The portal sheet gained the five shell class names `SectionEditor` styles itself with (`primary-btn`, `ghost-btn`, `auth-label`, `mds-note`, `row-actions`) — re-implemented in the portal palette, not copied from the brand's. **Blocked/remaining:** the revocation UX (`{"code":"access_revoked"}` + Download-your-draft, snapshotting the draft on revoke); `CreatorSubmittedEmail`; **`CreatorPublishNotificationEmail` keyed on `first_published_at`, not stage arrival**, or publish→unpublish→republish re-sends it; the retraction email; and the abandonment sweep on `turn_changed_at` (creator day 3, brand day 7). The email half is behind SES sandbox exit (case `178750875200560`). **Original scope, retained because the reasoning is still the spec for what is left:** mounts `SectionEditor.jsx` **unchanged** with creator-scoped callbacks; it is verified purely presentational (its only import is `../shell/sectionTypes`, with zero fetch, session or `brandId`). Pass `savedTemplates={[]}` and **omit `onSaveAsTemplate`**, since the gallery is metered against the brand's `PlanPolicy.Resource.SAVED_TEMPLATE` quota. New creator preview and rewrite endpoints; `/hand-back` moves **turn only, not stage** — the creator asserts done, the brand decides `ready_to_publish`. Revocation UX: `{"code":"access_revoked"}` plus Download-your-draft, snapshotting the draft on revoke so the brand keeps the work. `CreatorSubmittedEmail`; **`CreatorPublishNotificationEmail` — the acknowledgement asked for — keyed on `first_published_at`, not stage arrival**, or publish→unpublish→republish re-sends it; plus a retraction email if a page a creator was emailed about is unpublished. Abandonment sweep on `turn_changed_at`: creator reminder day 3, brand nudge day 7. **Extract `@influencer/ui`** — a third copy of `SectionEditor` is where duplication stops being tolerable, and CLAUDE.md already names this as the intended repair | 6 | The core of the ask. **Ghosting is the modal outcome in real creator marketing** and all three candidate designs assumed forward motion, so the abandonment sweep, the retraction email and the revocation UX are what make this survive contact with reality. Needs `OP-06` |
