package com.influencer.workflow.security;
...
public class ServiceTokenFilter extends OncePerRequestFilter {
    ...
    public ServiceTokenFilter(@Value("${workflow.service-token:}") String expectedToken) {
        this.expectedToken = expectedToken;
        if (expectedToken == null || expectedToken.isBlank()) {
            log.error("workflow.service-token is not configured. The Workflow service will reject all requests. "
                    + "Set workflow.service-token here and web-experience.workflow-service-token on the gateway to the same value.");
        }
    }
        ...
        if (!isAuthorized(request)) {
            ...
            response.getWriter().write(
                    "{\"status\":401,\"error\":\"Unauthorized\","
                            + "\"message\":\"A valid service token is required to call the Workflow service\"}");
            return;
        }
